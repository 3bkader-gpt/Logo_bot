...
MAX_FILE_SIZE_MB = 5  # الحد الأقصى لحجم الصورة 5 ميجابايت
...
# تم التعديل هنا لاستخدام logging.handlers مباشرة
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if os.path.exists(LOGO_CURRENT):
        await update.message.reply_text(f"اللوجو الحالي محفوظ هنا: {os.path.abspath(LOGO_CURRENT)}")
    elif os.path.exists(LOGO_ORIGINAL):
        await update.message.reply_text(f"اللوجو الأصلي محفوظ هنا: {os.path.abspath(LOGO_ORIGINAL)}")
    else:
...
async def set_size(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if 5 <= percentage <= 50:
            context.chat_data['custom_logo_scale'] = percentage / 100
            await update.message.reply_text(f"تم ضبط حجم اللوجو على {percentage}% من عرض الصورة.")
        else:
...
# أوامر إدارة المستخدمين (للمالكين فقط)
async def add_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if new_user_id not in allowed_users:
            ...
            await update.message.reply_text(f"تم إضافة المستخدم {new_user_id} بنجاح.")
        else:
...
async def handle_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.edit_message_caption(caption="حصل خطأ أثناء الحفظ.")
        await return_to_main_menu(query.message.chat_id, context)

# دالة تعيين اللوجو (للمالكين فقط)
...
# دالة إعادة تعيين اللوجو (للمالكين فقط)
...
async def watermark_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in OWNERS and user_id not in allowed_users:
        return
    await update.message.reply_text("ابعتلي الصورة:", reply_markup=ReplyKeyboardRemove())
...
async def receive_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'setting_logo' in context.chat_data:
        if not message.photo:
            await message.reply_text("ابعت صورة.")
            return
        try:
            ...
            await message.reply_text(f"حصل خطأ: {str(e)}", reply_markup=ReplyKeyboardRemove())
            context.chat_data.pop('setting_logo')
            await return_to_main_menu(message.chat_id, context)
        return
    ...
    if not message.photo:
        await message.reply_text("ابعت صورة.", reply_markup=ReplyKeyboardRemove())
        await return_to_main_menu(message.chat_id, context)
        return
...
async def style_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'photo_path' not in context.chat_data or 'photo_time' not in context.chat_data:
        await update.message.reply_text("ابعت صورة جديدة.", reply_markup=ReplyKeyboardRemove())
    ...
    if time.time() - context.chat_data['photo_time'] > STYLE_TIMEOUT_SEC:
        ...
        await update.message.reply_text("انتهى وقت الاختيار.", reply_markup=ReplyKeyboardRemove())
        await return_to_main_menu(chat_id, context)
        return

    choice = update.message.text.strip()
    choice_map = {
        '1️⃣ ركن أسفل يمين': '1', '2️⃣ منتصف اليمين': '2',
    ...
    try:
        ...
        scale = context.chat_data.get('custom_logo_scale', 0.2)
        lw = int(bw * scale)
        lh = int(lw * logo.height / logo.width)
        logo_resized = logo.resize((lw, lh), Image.Resampling.LANCZOS)
