...
async def set_size(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if 5 <= percentage <= 50:
            ...
            user_settings[user_id_str]['custom_logo_scale'] = percentage / 100
...
async def receive_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.chat_data.get('setting_logo'):
        ...
        try:
            ...
            context.chat_data.pop('setting_logo', None)
            await return_to_main_menu(message.chat_id, context)
        return

    if not context.chat_data.get('waiting_for_photo'):
        return

    if not (message.photo or (message.document and message.document.mime_type in ['image/jpeg', 'image/png'])):
        await message.reply_text("ابعت صورة أو ملف PNG/JPEG.", reply_markup=ReplyKeyboardRemove())
        await return_to_main_menu(message.chat_id, context)
        return
